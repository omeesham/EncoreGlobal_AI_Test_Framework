...
- Report shape: every step at the top level of a test body is numbered "Step 1: …", "Step 2: …" automatically — the `@step(...)` decorators and the `phase()` / `verify()` helpers in `src/fixtures/report-steps.ts` share one counter, and hook bodies are excluded so setup never spends "Step 1". A spec that only calls page objects is therefore already numbered. What conversion adds is NAMING: `phase()` where several actions form one business step, `verify()` around each assertion cluster (a bare `expect` reaches the report as "Expect toBe"). Run `npm run check:steps` to see what still reports unnamed. `tests/add-product-code/add-product-code.spec.ts` and `tests/item-search/*.spec.ts` are the reference implementations.
