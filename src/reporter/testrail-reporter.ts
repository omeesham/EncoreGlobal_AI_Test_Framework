// TestRail reporter — pushes run results to TestRail after the suite finishes.
//
// Enabled ONLY when TESTRAIL_ENABLED=true and the connection vars are set
// (see .env.testrail.example). Never fails the build: every TestRail error is
// reported as a warning and the local reports (HTML/Allure/JUnit) are untouched.
//
// Case mapping, in priority order per test:
//   1. Explicit TestRail id in the title or a tag:  "C123: …", "[C123] …", "@C123"
//   2. TCMT display-id token in the title (e.g. "TC-CPR-SRC-019") matched
//      against TestRail case titles containing the same token — works out of
//      the box when the cases were imported into TestRail from TCMT exports.
// Unmatched tests are listed in the summary, never dropped silently.
//
// Sprint roll-up: set TESTRAIL_MILESTONE_ID to the current sprint's milestone
// and every run attaches to it, so TestRail reports progress per sprint.
...
const TCMT_TOKEN_RE = /\bTC-[A-Z0-9]+(?:-[A-Z0-9]+)*-\d+\b/;

interface Collected {
  title: string;
  explicitId: number | null;
  tcmtToken: string | null;
  statusId: number;
  comment: string;
  elapsed: string;
}
...
export default class TestRailReporter implements Reporter {
  ...
  onTestEnd(test: TestCase, result: TestResult): void {
    ...
    this.collected.set(test.id, {
      ...
      tcmtToken: TCMT_TOKEN_RE.exec(test.title)?.[0] ?? null,
      statusId,
      comment: lines.join('\n'),
      elapsed: `${seconds}s`,
    });
  ...
  async onEnd(): Promise<void> {
    ...
    try {
      ...
      // Resolve TCMT tokens → TestRail case ids by scanning case titles once.
      const needsLookup = [...this.collected.values()].some((c) => !c.explicitId && c.tcmtToken);
      const tokenToCase = new Map<string, number>();
      if (needsLookup) {
        for (const c of await client.getCases(projectId, suiteId)) {
          const token = TCMT_TOKEN_RE.exec(c.title)?.[0];
      ...
      for (const c of this.collected.values()) {
        const caseId = c.explicitId ?? (c.tcmtToken ? tokenToCase.get(c.tcmtToken) : undefined);
        if (!caseId) {
          unmatched.push(c.title);
          continue;
        }
