...
test.describe('Item Search View Product Code — toolbar and dialog @item-search @product-code @view-product-code', () => {
  ...
  test('TC-ISR-PCD-001: Selecting a row reveals the View Product Code button and its segment arrow', async ({ dependencyGate }) => {
    dependencyGate([]);
  ...
  test('TC-ISR-PCD-006: Closing the dialog with a Name edit discards it silently', async ({ dependencyGate }) => {
    dependencyGate([]);
    await openAutomationItem();
    await expect(pc.dialogNameBox()).toHaveValue(ISR_AUTOMATION_ITEM.name);
  ...
  test('TC-ISR-PCD-007: The dialog shows the values of the selected row', async ({ dependencyGate }) => {
    dependencyGate([]);
    await pc.searchFor(ISR_AUTOMATION_ITEM.name, (n) => n === 1);
  ...
  test('TC-ISR-PCD-008: Every close path discards a dirty form silently', async ({ dependencyGate }) => {
    dependencyGate([]);
    await openAutomationItem();
    await pc.clickDialogTab('Translations');
  ...
  test('TC-ISR-PCD-009: A dirty Item-tab edit survives a round trip through the other tabs', async ({ dependencyGate }) => {
    dependencyGate([]);
    await openAutomationItem();
...
test.describe('Item Search View Product Code — segments and fields @item-search @product-code @view-product-code', () => {
  ...
  test('TC-ISR-PCD-010: Each segment shows the sections from Category down to its own level', async ({ dependencyGate }) => {
    dependencyGate([]);
  ...
  test('TC-ISR-PCD-011: Each segment carries only the fields that belong to its level', async ({ dependencyGate }) => {
    dependencyGate([]);
  ...
  test('TC-ISR-PCD-012: Names above the edited level are hierarchy dropdowns and its own name is text', async ({ dependencyGate }) => {
    dependencyGate([]);
    const row = await readAutomationRow();
  ...
  test('TC-ISR-PCD-014: History and Translations tabs exist on every segment and name the segment', async ({ dependencyGate }) => {
    dependencyGate([]);
...
test.describe('Item Search View Product Code — Item field rules @item-search @product-code @view-product-code', () => {
  ...
  test('TC-ISR-PCD-015: Name and Item Description stop at 50 characters and Oracle Item Number at 10', async ({ dependencyGate }) => {
    dependencyGate([]);
    await openAutomationItem();
  ...
  test('TC-ISR-PCD-016: An over-length value that bypasses the typing limit is refused', async ({ dependencyGate }) => {
    dependencyGate([]);
    await openAutomationItem();
    const nameBox = pc.dialogNameBox();
    const descriptionBox = pc.dialogDescriptionBox();
  ...
  test('TC-ISR-PCD-017: Clearing Name or Item Description marks it invalid and holds Save', async ({ dependencyGate }) => {
    dependencyGate([]);
    await openAutomationItem();
    const nameBox = pc.dialogNameBox();
    const descriptionBox = pc.dialogDescriptionBox();
  ...
  test('TC-ISR-PCD-018: A spaces-only Name is refused and a padded Name is saved without the spaces', async ({ dependencyGate }) => {
    dependencyGate([]);
    await openAutomationItem();
    const nameBox = pc.dialogNameBox();
  ...
  test('TC-ISR-PCD-019: A 50-character Name saves and reads back complete', async ({ dependencyGate }) => {
    dependencyGate([]);
  ...
  test('TC-ISR-PCD-020: Oracle Item Number is optional and accepts letters up to 10 characters', async ({ dependencyGate }) => {
    dependencyGate([]);
    await openAutomationItem();
    const oracleBox = pc.dialogOracleItemNumberBox();
  ...
  test('TC-ISR-PCD-021: Special characters in Name and Item Description save and show as plain text', async ({ dependencyGate }) => {
    dependencyGate([]);
    await openAutomationItem();
  ...
  test('TC-ISR-PCD-022: The own-name box of every upper segment also stops at 50 characters', async ({ dependencyGate }) => {
    dependencyGate([]);
    await selectAutomationItem();
    const ownNameBox: Record<string, string> = {
  ...
  test('TC-ISR-PCD-023: A Name edit enables Save on the first open and again after reopening', async ({ dependencyGate }) => {
    dependencyGate([]);
  ...
  test('TC-ISR-PCD-024: Typing the original Name back disables Save again', async ({ dependencyGate }) => {
    dependencyGate([]);
    await openAutomationItem();
...
test.describe('Item Search View Product Code — Item save @item-search @product-code @view-product-code', () => {
  ...
  test('TC-ISR-PCD-025: Editing the three Item text boxes saves and refreshes the grid row', async ({ dependencyGate }) => {
    dependencyGate([]);
    test.setTimeout(420_000);
  ...
  test('TC-ISR-PCD-026: Changing the Item Product Type resets its Service Type and saves', async ({ dependencyGate }) => {
    dependencyGate([]);
  ...
  test('TC-ISR-PCD-027: The Product Type list offers ten types and filters the Service Type list', async ({ dependencyGate }) => {
    dependencyGate([]);
    expect(await pc.readComboValue(await pc.ownProductTypeCombo())).toBe(ISR_AUTOMATION_ITEM.productType);
  ...
  test('TC-ISR-PCD-028: Product Organization offers the countries, saves and clears again', async ({ dependencyGate }) => {
    dependencyGate([]);
    test.setTimeout(420_000);
  ...
  test('TC-ISR-PCD-029: Choosing another Sub Class re-parents the item and the grid row follows', async ({ dependencyGate }) => {
    dependencyGate([]);
  ...
  test('TC-ISR-PCD-030: Deactivating and reactivating the item through Active and Save', async ({ dependencyGate }) => {
    dependencyGate([]);
    test.setTimeout(420_000);
  ...
  test('TC-ISR-PCD-031: A double click on Save sends one update and shows one message', async ({ dependencyGate }) => {
    dependencyGate([]);
  ...
  test('TC-ISR-PCD-032: A save whose request fails keeps the edits and can be retried', async ({ dependencyGate }) => {
    dependencyGate([]);
    const edited = `${ISR_AUTOMATION_ITEM.name}X`;
    await pc.appendToBox(pc.dialogNameBox(), 'X');
    await expectSaveEnabled(true);
...
test.describe('Item Search View Product Code — hierarchy saves @item-search @product-code @view-product-code', () => {
  ...
  test('TC-ISR-PCD-033: Renaming the Sub Class saves and every item under it shows the new name', async ({ dependencyGate }) => {
    dependencyGate([]);
    test.setTimeout(420_000);
    await renameLevelAndRestore('Sub Class', 'Enter sub-class name', 'Sub Class', ISR_ZZ_CHAIN.subClass);
  });

  test('TC-ISR-PCD-034: Renaming the Class saves and reads back', async ({ dependencyGate }) => {
    dependencyGate([]);
    test.setTimeout(420_000);
    await renameLevelAndRestore('Class', 'Enter class name', 'Class', ISR_ZZ_CHAIN.className);
  });

  test('TC-ISR-PCD-035: Renaming the Sub Category saves and reads back', async ({ dependencyGate }) => {
    dependencyGate([]);
    test.setTimeout(420_000);
    await renameLevelAndRestore('Sub Category', 'Enter sub-category name', 'Sub Category', ISR_ZZ_CHAIN.subCategory);
  });

  test('TC-ISR-PCD-036: Renaming the Category saves only the Category level', async ({ dependencyGate }) => {
    dependencyGate([]);
    test.setTimeout(420_000);
    await pc.openSegment('Category');
  ...
  test('TC-ISR-PCD-037: Changing the Service Type at the Sub Class level saves and reads back', async ({ dependencyGate }) => {
    dependencyGate([]);
    await pc.openSegment('Sub Class');
  ...
  test('TC-ISR-PCD-038: Flipping Barcodeable on the Sub Class propagates to its items', async ({ dependencyGate }) => {
    dependencyGate([]);
    await pc.openSegment('Sub Class');
    expect(await pc.readDialogCheckboxLabels()).toEqual(['Barcodeable', 'Active']);
    expect(await pc.isBoxDisabled(pc.barcodeableBox())).toBe(false);
  ...
  test('TC-ISR-PCD-040: Choosing an organization on the Sub Class saves and reaches its items', async ({ dependencyGate }) => {
    dependencyGate([]);
...
test.describe('Item Search View Product Code — Translations @item-search @product-code @view-product-code', () => {
  ...
  test('TC-ISR-PCD-041: Each translation box accepts 256 characters and refuses the 257th', async ({ dependencyGate }) => {
    dependencyGate([]);
    await pc.clickDialogTab('Translations');
  ...
  test('TC-ISR-PCD-042: Editing one language\'s Name saves and reads back after reopening', async ({ dependencyGate }) => {
    dependencyGate([]);
    await pc.clickDialogTab('Translations');
  ...
  test('TC-ISR-PCD-043: A blank translation saves clean and reads back blank', async ({ dependencyGate }) => {
    dependencyGate([]);
    await pc.clickDialogTab('Translations');
  ...
  test('TC-ISR-PCD-044: An Item edit and a translation edit in one Save both persist', async ({ dependencyGate }) => {
    dependencyGate([]);
...
test.describe('Item Search View Product Code — History grid @item-search @product-code @view-product-code', () => {
  ...
  test('TC-ISR-PCD-045: The History grid is read-only', async ({ dependencyGate }) => {
    dependencyGate([]);
    const rowsBefore = await readHistoryRowsFresh();
  ...
  test('TC-ISR-PCD-046: An Item save adds a new top row to the History grid', async ({ dependencyGate }) => {
    dependencyGate([]);
  ...
  test('TC-ISR-PCD-047: Translation-only and organization-only saves each add a History row', async ({ dependencyGate }) => {
    dependencyGate([]);
    test.setTimeout(420_000);
    const rowsBefore = await readHistoryRowsFresh();
    const topBefore = rowsBefore[0] ?? [];
  ...
  test('TC-ISR-PCD-048: History paging, sorting and column hiding work and hiding persists', async ({ dependencyGate }) => {
    dependencyGate([]);
    test.setTimeout(420_000);
  ...
  test('TC-ISR-PCD-049: The oldest History row is the create row', async ({ dependencyGate }) => {
    dependencyGate([]);
...
test.describe('Item Search View Product Code — Sub Class organization prompt @item-search @product-code @view-product-code', () => {
  ...
  test('TC-ISR-PCD-050: Closing the items prompt unanswered keeps the change unsaved', async ({ dependencyGate }) => {
    dependencyGate([]);
  ...
  test('TC-ISR-PCD-051: Answering No while the item has no organization is refused and keeps the edits', async ({ dependencyGate }) => {
    dependencyGate([]);
    await chooseOnSubClass(ISR_ORG_PICK);
  ...
  test('TC-ISR-PCD-052: Answering No when the item already fits updates only the Sub Class', async ({ dependencyGate }) => {
    dependencyGate([]);
    test.setTimeout(420_000);
  ...
  test('TC-ISR-PCD-053: Removing one organization from the Sub Class reaches its items', async ({ dependencyGate }) => {
    dependencyGate([]);
    test.setTimeout(420_000);
    await pc.openSegment('Sub Class');
    await pc.openOrgList('Sub Class');
  ...
  test('TC-ISR-PCD-054: Clearing the Sub Class organization to None saves at once and leaves its items as they are', async ({ dependencyGate }) => {
    dependencyGate([]);
    await chooseOnSubClass(ISR_ORG_PICK);
    await pc.clickSaveExpectItemsPrompt();
    await answerYesAndConfirm();
...
test.describe('Item Search View Product Code — segment entries and unsaved picks @item-search @product-code @view-product-code', () => {
  ...
  test('TC-ISR-PCD-055: The Category entry of an item without a category is offered but opens nothing', async ({ dependencyGate }) => {
    dependencyGate([]);
  ...
  test('TC-ISR-PCD-056: Picking another Sub Class keeps the saved identifier shown until the save', async ({ dependencyGate }) => {
    dependencyGate([]);
    await openAutomationItem();
