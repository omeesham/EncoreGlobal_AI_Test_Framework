...
test.describe('Location Business Types @locations @business-types', () => {
  ...
  test.beforeEach(async ({ locationBusinessTypesPage }) => {
    // The restore below can run up to three save-and-reload cycles when a previous run left this
    // office dirty, which is well past the default 30s hook budget. Calling this inside the hook
    // raises the budget for the hook itself; test.setTimeout in a test body does not reach back
    // here, so without it the self-heal path times out in exactly the case it exists to handle.
  ...
  test('TC-LOC-BTY-016: Unsaved Changes Prompt Discard Leaves The Page And Drops The Edit', async ({ locationBusinessTypesPage, dependencyGate }) => {
    dependencyGate(['TC-LOC-BTY-001']);
    // Three full page loads happen in this case -- the one in the hook plus the two below -- and
    // each currently costs this application around 45 seconds, so 90s cannot cover the test and it
    // fails on the second load every time. The budget is set above the measured worst case rather
    // than dropping a navigation the case depends on.
    test.setTimeout(180_000);
    await locationBusinessTypesPage.navigateFresh(OFFICE_NO);
    await locationBusinessTypesPage.toggleCheckbox('chkBusinessTypeExpoServices');
    await expect.poll(() => locationBusinessTypesPage.isSaveEnabled(), { timeout: 5_000 }).toBe(true);
