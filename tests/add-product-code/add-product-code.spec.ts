 ...
 *
 * The View Product Code dialog and its tabs are a separate sub-task with their own spec and
 * their own test-case numbering sequence; this file's cases are numbered on their own.
 */
...
test.describe('Item Search Add Product Code @item-search @product-code @add-product-code', () => {
  ...
  test('TC-ISR-APC-001: Add Product Code opens a required-empty form with Save held back', async ({ dependencyGate }) => {
    dependencyGate([]);
  ...
  test('TC-ISR-APC-002: Choosing a Product Type unlocks and filters Service Type', async ({ dependencyGate }) => {
    dependencyGate([]);
    await searchAndSelect();
    await pc.openAddDialog();
  ...
  test('TC-ISR-APC-003: The Add segment menu opens per-segment forms', async ({ dependencyGate }) => {
    dependencyGate([]);
    await searchAndSelect();
  ...
  test('TC-ISR-APC-004: A completed Add Product Code form saves and the new code is found again', async ({ dependencyGate }) => {
    dependencyGate([]);
    await searchAndSelect();
  ...
  test('TC-ISR-APC-005: The text fields stop accepting input at their maximum lengths', async ({ dependencyGate }) => {
    dependencyGate([]);
    await searchAndSelect();
    await pc.openAddDialog();
  ...
  test('TC-ISR-APC-006: An over-length value that bypasses the typing limit cannot be saved', async ({ dependencyGate }) => {
    dependencyGate([]);
    await searchAndSelect();
    await pc.openAddDialog();
  ...
  test('TC-ISR-APC-007: A name at exactly the maximum length saves and reads back complete', async ({ dependencyGate }) => {
    dependencyGate([]);
    await searchAndSelect();
  ...
  test('TC-ISR-APC-008: Closing the Add dialog with a part-filled form discards it silently', async ({ dependencyGate }) => {
    dependencyGate([]);
    await searchAndSelect();
    await pc.openAddDialog();
