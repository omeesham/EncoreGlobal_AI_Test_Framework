...
test.describe('Service Charge History', () => {
  ...
  test('TC-SVC-HIS-012: Sorting the History grid by Modified On via the column header dropdown reorders rows', async ({
    ...
    // Falls back to an empty string rather than asserting non-null, so a missing row 0 fails the
    // length check loudly instead of throwing an unhelpful type error.
